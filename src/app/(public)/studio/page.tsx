...
export default function StudioPage() {
  ...
  async function genPhoto() {
    ...
    setGenerating(false)
    toast.success('Photo generated by ' + (STUDIO_AGENTS.find(a => a.slug === item.agent)?.name || 'Agent'))
  }

  async function genVideo() {
    if (!prompt.trim()) { toast.error('Describe the video'); return }
    setGenerating(true)
    toast.info(`Video ${duration}s in progress (async polling)...`)
    ...
    setGallery(prev => [item, ...prev])
    setGenerating(false)
    toast.success(`Video ${duration}s generated`)
  }
      ...
      <div className="max-w-6xl mx-auto px-4 py-12">
        ...
        <div className="text-center mb-12">
          ...
          <h1 className="text-4xl font-bold text-white font-[family-name:var(--font-playfair)] mb-3">
            Studio <span className="text-[#E50914]">AI Creative</span>
          </h1>
          <p className="text-gray-400 max-w-xl mx-auto">
            Generate posters, storyboards, concept art, VFX and videos with 7 specialized cinema agents.
          </p>
          ...
          <div className="mt-6 flex gap-3 justify-center">
            <a href="/studio/guided" className="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-emerald-500/20 bg-emerald-500/5 text-sm text-emerald-400 hover:bg-emerald-500/10 transition-colors">
              <Wand2 className="h-4 w-4" /> Guided mode
            </a>
        ...
        {tab === 'photo' && (
          <div className="space-y-6">
            <div className="rounded-2xl border border-gray-800 bg-gray-900/50 p-6">
              ...
              <textarea value={prompt} onChange={e => setPrompt(e.target.value)} placeholder="Describe the cinematic image to generate..." rows={3} className="w-full rounded-xl border border-gray-700 bg-gray-800 px-4 py-3 text-sm text-white placeholder-gray-500 focus:border-[#E50914] focus:outline-none resize-none" />
            </div>

            {/* Category */}
            <div>
              <label className="text-xs text-gray-400 mb-2 block">Cinema category</label>
        ...
        {tab === 'queue' && (
          <div className="rounded-2xl border border-gray-800 bg-gray-900/50 p-8 text-center">
            ...
            <p className="text-xs text-gray-600 mt-1">Agent creation requests will appear here</p>
          </div>
        )}
